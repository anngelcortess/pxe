...
echo ""
echo -e "${BLUE}O instálalo como servicio systemd de usuario (recomendado):${NC}"
echo ""
echo -e "${GREEN}  mkdir -p ~/.config/systemd/user${NC}"
echo -e "${GREEN}  sed \"s|REPO_PATH|\$(pwd)|g\" vbox-api.service > ~/.config/systemd/user/vbox-api.service${NC}"
echo -e "${GREEN}  systemctl --user daemon-reload${NC}"
echo -e "${GREEN}  systemctl --user enable --now vbox-api${NC}"
echo ""
