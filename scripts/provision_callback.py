...
# Ruta al aprovisionador Python (provisioner.py), relativa a este script
ORCHESTRATE_SCRIPT = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'provisioner.py')
...
class CallbackHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _run_finalize(self, node_name):
        """Invocar provisioner.py finalize-node <nombre>."""
        if not os.path.exists(ORCHESTRATE_SCRIPT):
            log.error(f"No se encontró provisioner.py en: {ORCHESTRATE_SCRIPT}")
            return
...
def main():
    ...
    log.info(f"Aprovisionador:         {ORCHESTRATE_SCRIPT}")
